...
public class VendingMachine {
	...
	public void addItem(VendingMachineItem item, String code)
			// in the previous code, if an invalid code was entered, it should've handled the exception.
			throws VendingMachineException {
		if(code.equals(A_CODE)||code.equals(B_CODE)|| code.equals(C_CODE) || code.equals(D_CODE)) {
	...
	public VendingMachineItem removeItem(String code) throws VendingMachineException {
		// in the code below if an invalid code is entered, it would not be handled.
		// int slotIndex = getSlotIndex(code);
		// VendingMachineItem item = itemArray[slotIndex];
		// itemArray[slotIndex] = null;
		
		VendingMachineItem item = null;
		if(code.equals(A_CODE)||code.equals(C_CODE) || code.equals(D_CODE)){
			int slotIndex = getSlotIndex(code);
			item = itemArray[slotIndex];
			itemArray[slotIndex] = null;
			if ( item == null) {
	...
	public boolean makePurchase(String code) {
		...
		// in the previous code below if an invalid code is entered, it would not be handled.
		if(code.equals(A_CODE)||code.equals(B_CODE)||code.equals(C_CODE)||code.equals(D_CODE)){
		...
		else{ throw new VendingMachineException(VendingMachine.INVALID_CODE_MESSAGE);
		}
		return returnCode;
	}
	...
	/* The pre condition allocates balance > = 0, but the code doesn't check for the precondition.
	It assigns the balance value to change.
	*/
